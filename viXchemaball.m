function h = viXchemaball(r, lbls, ccolor, ncolor)

% ViXCHEMABALL Plots correlation matrix as a viXchemaball
%
%   ViXCHEMABALL(R) R is a square numeric matrix with values in [0,1].
%
%                 NOTE: only the off-diagonal lower triangular section of R is
%                       considered, i.e. tril(r,-1).
%
%   ViXCHEMABALL(..., LBLS, CCOLOR, NCOLOR) Plot viXchemaball with optional 
%                                         arguments (accepts empty args).
%                                               
%       - LBLS      Plot a viXchemaball with custom labels at each node.
...
%
%
%   H = ViXCHEMABALL(...) Returns a structure with handles to the graphic objects
%
...
%   viXchemaball(x, repmat(('a':'j')',1,2))
%   viXchemaball(x, {'Hi','how','is','your','day?', 'Do','you','like','viXchemaballs?','NO!!'})
%
%   % Customize curve colors
%   viXchemaball([],[],[1,0,1;1 1 0])
%
...
%
% Additional features:
% - <a href="matlab: web('http://www.mathworks.com/matlabcentral/fileexchange/42279-viXchemaball','-browser')">FEX viXchemaball page</a>
% - <a href="matlab: web('http://www.stackoverflow.com/questions/17038377/how-to-visualize-correlation-matrix-as-a-viXchemaball-in-matlab/17111675','-browser')">Origin: question on Stackoverflow.com</a>
% - <a href="matlab: web('https://github.com/GuntherStruyf/matlab-tools/blob/master/viXchemaball.m','-browser')">Schemaball by Gunther Struyf</a>
%
...
sz = size(r);
if nargin < 2 || isempty(lbls1);     lbls1   = cellstr(reshape(sprintf('%-4d',1:sz(1)),4,sz(1))');  end
if nargin < 4 || isempty(ncolor);   ncolor = [0 0 1];                                             end

% R
if ~isnumeric(r) || any(abs(r(:)) > 1) || sz(1) ~= sz(2) || numel(sz) > 2 || sz(1) == 1
    error('viXchemaball:validR','R should be a square numeric matrix with values in [0, 1].')
end

% Lbls
if (~ischar(lbls1) || size(lbls1,1) ~= sz(1)) && (~iscellstr(lbls1) || ~isvector(lbls1) || length(lbls1) ~= sz(1))
    error('viXchemaball:validLbls','LBLS should either be an M by N char array or a cellstring of length M, where M is size(R,1).')
end
if ischar(lbls1)
    lbls1 = cellstr(lbls1);
end
...
if nargin < 3 || isempty(ccolor);
    ...
    if ~isnumeric(ccolor) || szC(2) ~= 3  || szC(1) > 2
        error('viXchemaball:validCcolor','CCOLOR should be a 1 by 3 or 2 by 3 numeric matrix with RGB colors.')
    elseif szC(1) == 1
...
if ~isnumeric(ncolor) || szN(2) ~= 3  || szN(1) > 1
    error('viXchemaball:validNcolor','NCOLOR should be a single RGB color, i.e. a numeric row triplet.')
end
...
ipos       = x > 0;
s.t        = zeros(sz(1),1);
s.t( ipos) = text(x( ipos)*1.1, y( ipos)*1.1, lbls2( ipos),'Color',tcolor);
...
ipos       = x > 0;
s.t        = zeros(sz(1),1);
s.t( ipos) = text(x( ipos)*1.6, y( ipos)*1.6, lbls1( ipos),'Color',tcolor);
set(s.t( ipos),{'Rotation'}, num2cell(theta(ipos)'/tau*360))
s.t(~ipos) = text(x(~ipos)*1.6, y(~ipos)*1.6, lbls1(~ipos),'Color',tcolor);
set(s.t(~ipos),{'Rotation'}, num2cell(theta(~ipos)'/tau*360 - 180),'Horiz','right')
...
% set(gcf,'Position',[1 1 1400 1400]);
axis equal
